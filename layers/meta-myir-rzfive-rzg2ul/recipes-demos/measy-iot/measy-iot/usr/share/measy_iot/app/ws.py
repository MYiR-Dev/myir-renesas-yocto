...
def _get_wifi_statu():
    ...
    (status, wlan) = subprocess.getstatusoutput(command)
    ...
    if "off/any" in wifi_connect :
        ...
        if wifi_is_connect == 0:
            ...
            os.system(command)
            wifi_is_connect = 1
            ifdata = Ifcfg(subprocess.getoutput('ifconfig'))
...
def parse_ip_config(res):
    ...
    for i in res:
        (path, params) = i
        result_list.append((path, params))
        return result_list
...
def wifi_connect(wifi_info):
    ...
    if encryption =='Open':
        ...
        (status, wifi_id) = subprocess.getstatusoutput(command)
